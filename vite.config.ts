...
export default defineConfig(({ mode }) => {
  ...
  return {
    base: process.env.FIGMA_PUBLIC_URL ? `${process.env.FIGMA_PUBLIC_URL}/` : '/us-digital-environments-demo/',
    build: {
...
function figmaSiteConfiguration(config: FigmaSiteConfiguration): Plugin {
  ...
  const title = config.title ?? "US Digital Environments"
  const description = config.description ?? ''
  const favicon = config.icons?.icon ?? '/src/imports/Logo-Transparent.png'
